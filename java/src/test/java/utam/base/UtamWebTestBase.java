...
public abstract class UtamWebTestBase {
  ...
  protected final void setupChrome(ApiService mxService) {
    MutableCapabilities caps = mxService.getBaseCaps();
    driver = new RemoteWebDriver(mxService.getURL(), caps);
    setUtam(driver);
    mxService.processSession(driver);
    mxService.waitForLiveVideo();
  }
   ...
   */
  final UtamLoader setUtam(RemoteWebDriver driver) {
    this.driver = driver;
